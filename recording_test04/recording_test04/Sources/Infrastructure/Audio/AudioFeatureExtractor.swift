...
final class AudioFeatureExtractor {
  ...
  // Python学習時は2.0秒波形へtorch.stftのreflect paddingを適用し、
  // 173フレームを作る。
  ...
  private func loadMelFilters() {
    guard let path = Bundle.main.path(forResource: "mel_filters", ofType: "json"),
      ...
      let weightsDouble = json["weights"] as? [Double]
    else {
      AppLogger.audio.error("mel_filters.jsonが見つからないか、内容が不正です")
      return
    }
  ...
  private func processFeatures(pcmL: [Float], pcmR: [Float]) -> MLMultiArray? {
    ...
    for t in 0..<targetFrames {
      ...
      for m in 0..<nMels {
        ...
        let leftDecibel = 10.0 * log10(max(melPwrL, 1e-10))
        let rightDecibel = 10.0 * log10(max(melPwrR, 1e-10))
        logMelL[m][t] = leftDecibel
        logMelR[m][t] = rightDecibel
        ild[m][t] = leftDecibel - rightDecibel
    ...
    do {
      ...
      AppLogger.audio.error("特徴量配列の生成に失敗しました: \(error.localizedDescription)")
      return nil
    }
  }
