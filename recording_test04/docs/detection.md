...
1. Settingsで端末向き、前面／背面マイク、警告音を選択します。
2. Detectingsタブで`Start`を押します。
3. 必要に応じてGround Truthを選択します。
4. 実験用ビープを確認する場合は`Beep Test`を押します。
5. `Stop`を押すとWAVとCSVを保存します。

Ground Truthは`FalseDetect`または0°から315°まで45°刻みで選択できます。選択値は
ビープ検出時にそのイベントへ固定されます。
