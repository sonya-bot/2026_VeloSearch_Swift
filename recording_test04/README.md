...
`recording_test04`は、iPhoneのステレオマイクを使った録音、テスト音源を利用した
モニタリング録音、Core MLによる8方向の音源定位、計測データ管理を行うiOSアプリです。
...
| Recordings | 44.1 kHz、16-bit PCM、ステレオWAVを録音します。 |
| Monitorings | 選択したテスト音源を再生しながらステレオ録音します。 |
| Collections | DefaultとSceneごとに録音を一覧表示し、検索、削除、お気に入り、ZIP共有を行います。 |
| Player | WAVの再生、シーク、レベル表示、名称変更、メタデータ編集、共有を行います。 |
| Settings | 端末向き、使用マイク、警告音、モニタリング、デバッグ表示を設定します。 |

画面は4つのタブで構成されます。モニタリング設定がOFFの場合は2番目のタブが
Recordings、ONの場合はMonitoringsになります。
...
録音とCSVはアプリのDocuments配下へ保存されます。初回起動時に`Default`フォルダを作成し、
...
- モニタリングの入力デバイス選択は保存されますが、録音では内蔵マイクを使用します。
- 一部のモニタリング音源は選択肢のみで、対応するリソースが同梱されていません。
